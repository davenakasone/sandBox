...
`define SAFETY 50     // kill after this many of instructions, stops oo loop
...
module test_bench();
    ...
    initial begin
        ...
        `ifdef DEBUG_tb
            ...
            `ifdef LED_TEST
                $write("\n testing rotate LEDs ,");
                $write("  success rotating M[0x50] =   0->7->0...\n");
            `endif
    ...
    always @ (negedge clock) begin
        ...
        if (printing) begin
            ...
            `ifdef DEBUG_tb
                `ifdef LED_TEST
                    if (memwrite == 1'b1 && dataadr == 80) begin
                        $write("\tM[%2h] =  %1d\n", dataadr[7:0], writedata[4:0]);
                    end
                `else
                    $write(" %2d)  %4H_%4H  |  %4H_%4H  |  %1b  |  %4H_%4H  |  %4H_%4H        %6t ns\n",
                        idx,
        ...
        if (play_time < 0 || idx > `SAFETY) begin
            `ifdef DEBUG_tb
                `ifdef LED_TEST
                    $write("\n\n\tif you see the rotation, it is good\n");
                `else
