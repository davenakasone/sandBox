/*
    original "fresh.v" with corrections
*/
...
// only activate one at a time, include one for each instruction set
//`define ORIGINAL_INSTRUCTIONS 100
`define CHANGED_INSTRUCTIONS 101
...
module testbench();
    ...
    `ifdef ORIGINAL_INSTRUCTIONS
        // with original "memfile.dat", 4-state logic check, success means M[84] = 7
    ...
    `ifdef CHANGED_INSTRUCTIONS
        // with changed "memfile_CHANGED.dat", 4-state logic check, success means M[84] = 22
        always @ (negedge clk) begin
            if (memwrite == 1'b1 && access_complete == 1'b0) begin 
...
module imem (
    ...
    initial begin
        ...
        `endif
        `ifdef CHANGED_INSTRUCTIONS
            $readmemh ("memfile_CHANGED.dat", RAM);  // lines of test code in hex (changed)
        `endif
        `ifdef DEBUG_INITIAL
