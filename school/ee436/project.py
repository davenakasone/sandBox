...
def mystrip(plot_epef) :
    ...
    print(f"\tl =  {l0:7.3E}, ep_e unadjusted")
    print(f"\tl =  {lf:7.3E}, ep_e adjusted for target f = {ftarget/1.0e9:0.1f} GHz")
    
    print("\n# 4,  find attenuation :")
    a_d = calc_alphad(k0, epr, epe, delta)
    print(f"\talpha_d    =  {a_d:7.5f} Np/m  -->  {Npm_2_dBm(a_d):0.3E} dB/m")
    a_c = calc_alphac(Rs, Z0, W)
    print(f"\talpha_c    =  {a_c:7.5f} Np/m  -->  {Npm_2_dBm(a_c):0.3E} dB/m")
    loss = (Npm_2_dBm(a_c) + Npm_2_dBm(a_d)) * l0
    print(f"\ttotal loss =  {loss:7.3E} dB")
    
    print("\n# 5,  find the velocities :")
    vp = calc_vp(epe)
    beta = calc_beta(k0, epe)
    print(f"\tvp   = {vp:7.3E} m/s, quasi-static TEM phase velocity")
    print(f"\tbeta = {beta:7.3E} m/s, quasi-static TEM propagation constant")
    ...
    ft4 = calc_ft4(d, epr)
    print(f"\tf_operate =  {ftarget/1.0e9:7.3f} GHz, also target/design frequency")
    print(f"\tft1       =  {ft1/1.0e9:7.3f} GHz, TM1 cutoff, within 35% - 66% of fc1")
    print(f"\tft2       =  {ft2/1.0e9:7.3f} GHz, TE1 cutoff")
    print(f"\tft3       =  {ft3/1.0e9:7.3f} GHz, wide cutoff")
    print(f"\tft4       =  {ft4/1.0e9:7.3f} GHz, thick cutoff")
    
    print("\n# 7,  confirm the results :")
    lam = const.c / ftarget
    mlen = lam * multlam
    print(f"\td      =  {d:7.3E} m  ,  thickness of the dielectric")
    print(f"\tW      =  {W:7.3E} m  ,  width of the microstrip")
    print(f"\tlamda  =  {lam:7.3E} m  ,  wavelength")
    print(f"\tlength =  {mlen:7.3E} m  ,  how long to size the microstrip -->  {multlam} wavelengths")
    print(f"\tZ0     =  {Zchar_wd(epe, d, W):12.3f} ohms  ,  target = {Z0:0.3f} ohms")
