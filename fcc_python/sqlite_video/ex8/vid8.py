...
# 5, order the query for descending rowid
c.execute(f"SELECT rowid, * FROM {TAB_NAME} ORDER BY rowid DESC")
pkeys = c.fetchall()
for key in pkeys :
    print(f"{key}")
print("\n")

# 6, order the query for descending last name
c.execute(f"SELECT rowid, * FROM {TAB_NAME} ORDER BY last_name DESC")
pkeys = c.fetchall()
for key in pkeys :
    print(f"{key}")
print("\n")

# 7, order the query for ascending first name
c.execute(f"SELECT rowid, * FROM {TAB_NAME} ORDER BY first_name ASC")
pkeys = c.fetchall()
for key in pkeys :
    print(f"{key}")
print("\n")
