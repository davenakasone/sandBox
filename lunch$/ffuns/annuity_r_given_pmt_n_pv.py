...
(
    ...
    # get solutions
    r_sol = sympy.solve(PV_eq.subs([(PV_sym, present_value), (n_sym, number_of_periods), (P_sym, payment)]), r_sym)
    if r_sol is None :
    ...
    for rate in rate_list :
        pv = annuity_pv_given_pmt_n_r(payment, number_of_periods, rate)
        if abs(pv - present_value) < ALLOWABLE_DIFFERENCE :
            return rate
    return FAILED_CALCULATION_FLOAT


########~~~~~~~~END>  annuity_r_given_pmt_n_pv.py
